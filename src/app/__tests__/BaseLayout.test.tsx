...
import LayoutBody from '../../widgets/body/ui/LayoutBody';
import styles from '../../widgets/body/ui/LayoutBody.module.scss';
import AddressButtons from '../../shared/ui/addressFields/AddressButtons';
...
describe('LayoutBody component', () => {
  it('renders the main body correctly', () => {
    render(<LayoutBody />);

    const mainBodyElement = screen.getByRole('main');
    expect(mainBodyElement).toBeInTheDocument();
    expect(mainBodyElement).toHaveClass(styles.mainBody);
  });
});

describe('AddressButtons component', () => {
  const mockHandleFirstBtn = jest.fn();
  const mockHandleSecondBtn = jest.fn();
  ...
  it('calls the correct handler when a button is clicked', () => {
    render(
      <AddressButtons
        ...
        firstBtnText="Button 1"
        secondBtnText="Button 2"
        handleFirstBtn={mockHandleFirstBtn}
        handleSecondBtn={mockHandleSecondBtn}
      />,
    );

    const firstButton = screen.getByRole('button', { name: /Button 1/i });
    fireEvent.click(firstButton);
    expect(mockHandleFirstBtn).toHaveBeenCalledTimes(1);

    const secondButton = screen.getByRole('button', { name: /Button 2/i });
    fireEvent.click(secondButton);
    expect(mockHandleSecondBtn).toHaveBeenCalledTimes(1);
  });
});
